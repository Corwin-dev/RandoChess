...
class RandoChessApp {
    ...
    initialize() {
        ...
        if (
            this.seed === null &&
            this.pieces &&
            typeof this.pieces.__seed !== 'undefined'
        ) {
        ...
        if (this.uiManager) {
            ...
            this.uiManager.onTakebackClick(() => {
                try {
                    ...
                    if (this.currentController instanceof AIGameController) {
                        if (
                            typeof this.currentController.takeback ===
                            'function'
                        )
                        ...
                        if (
                            this.uiManager &&
                            typeof this.uiManager.setTakebackRequested ===
                                'function'
                        )
                            this.uiManager.setTakebackRequested(false, false);
                        this.takebackLocalRequest = false;
                        this.takebackOpponentRequest = false;
                        return;
                    }
                    ...
                    if (
                        this.currentController instanceof OnlineGameController
                    ) {
                        ...
                        if (this.takebackOpponentRequest) {
                            ...
                            try {
                                ...
                                console.warn(
                                    'Ignored error performing double takeback (game.js)',
                                    e
                                );
                            }
                            ...
                            if (
                                this.onlineSocket &&
                                this.onlineSocket.readyState === WebSocket.OPEN
                            ) {
                                this.onlineSocket.send(
                                    JSON.stringify({ type: 'TAKEBACK_PERFORM' })
                                );
                            }
                        ...
                        if (
                            ...
                            this.uiManager.setTakebackRequested(
                                true,
                                this.takebackOpponentRequest
                            );
                        if (
                            this.onlineSocket &&
                            this.onlineSocket.readyState === WebSocket.OPEN
                            ...
                            this.onlineSocket.send(
                                JSON.stringify({ type: 'TAKEBACK_REQUEST' })
                            );
                        }
                        return;
                    }
                    ...
                    console.warn(
                        'Ignored error in takeback click handler (game.js)',
                        e
                    );
                }
            });
            // Disable until a move is recorded
            /* global PieceSerializer, MultiplayerGameController */
            if (typeof this.uiManager.setTakebackEnabled === 'function')
            ...
            if (this.uiManager.onDrawClick) {
                this.uiManager.onDrawClick(() => {
                    try {
                        ...
                        if (
                            this.currentController instanceof AIGameController
                        ) {
                            // End as draw
                            try {
                                this.currentController.isActive = false;
                            } catch (e) { console.warn('Ignored error (game.js)', e); }
                            if (
                                this.currentController &&
                                this.currentController.engine
                            )
                                this.currentController.engine.gameOver = true;
                            if (this.uiManager)
                        ...
                        if (
                            this.currentController instanceof HotseatController
                        ) {
                            ...
                            if (
                                this.uiManager &&
                                typeof this.uiManager.setDrawRequested ===
                                    'function'
                            )
                                this.uiManager.setDrawRequested(true, false);
                            return;
                        }

                        // Online: send a request and highlight until opponent also requests.
                        if (
                            this.currentController instanceof
                            OnlineGameController
                        ) {
                            if (this.drawOpponentRequest) {
                                ...
                                if (
                                    this.onlineSocket &&
                                    this.onlineSocket.readyState ===
                                        WebSocket.OPEN
                                ) {
                                    this.onlineSocket.send(
                                        JSON.stringify({ type: 'DRAW_PERFORM' })
                                    );
                                }
                                if (this.uiManager) {
                                    this.uiManager.showMessage('🤝', 0);
                                    ...
                                    try {
                                        ...
                                        console.warn(
                                            'Ignored UI endmatch update error (game.js)',
                                            e
                                        );
                                    }
                                }
                            ...
                            if (
                                ...
                                this.uiManager.setDrawRequested(
                                    true,
                                    this.drawOpponentRequest
                                );
                            if (
                                this.onlineSocket &&
                                this.onlineSocket.readyState === WebSocket.OPEN
                                ...
                                this.onlineSocket.send(
                                    JSON.stringify({ type: 'DRAW_REQUEST' })
                                );
                            }
                            return;
                        }
                        ...
                        console.warn(
                            'Ignored error in draw click outer handler (game.js)',
                            e
                        );
                    }
                });
            ...
            if (this.uiManager.onForfeitClick) {
                this.uiManager.onForfeitClick(() => {
                    try {
                        ...
                        let resigning = null;
                        if (this.currentController.playerColor)
                            resigning = this.currentController.playerColor;
                        else if (
                            this.currentController.engine &&
                            this.currentController.engine.currentTurn
                        )
                            resigning =
                                this.currentController.engine.currentTurn;
                        ...
                        if (
                            ...
                            if (
                                ...
                                this.onlineSocket.send(
                                    JSON.stringify({ type: 'RESIGN', winner })
                                );
                            }
                        }
                        ...
                        if (this.uiManager) {
                            ...
                            try {
                                ...
                                console.warn(
                                    'Ignored UI endmatch update error in forfeit handler (game.js)',
                                    e
                                );
                            }
                        }
                        ...
                        console.warn(
                            'Ignored error in forfeit click handler (game.js)',
                            e
                        );
                    }
                });
                    ...
                    try {
                        if (
                            this.currentController instanceof
                                OnlineGameController &&
                            this.currentController.engine &&
                            this.currentController.engine.isGameOver &&
                            this.currentController.engine.isGameOver()
                        ) {
                            // mark local selection and send vote
                            this.rematchLocalSelection = 'roll';
                            if (this.uiManager && this.uiManager.rerollBtn) {
                                this.uiManager.rerollBtn.classList.add(
                                    'draw-requested'
                                );
                                this.uiManager.resetBtn &&
                                    this.uiManager.resetBtn.classList.remove(
                                        'draw-requested'
                                    );
                            }
                            if (
                        ...
                        this.seed = seedVal;
                        this.pieces =
                            PieceGenerator.generateRandomPieces(seedVal);
                        try {
                            if (
                                this.currentController instanceof
                                HotseatController
                        ...
                        console.warn(
                            'Ignored error in reroll click handler (game.js)',
                            e
                        );
                    }
                });
                    ...
                    try {
                        if (
                            ...
                            if (this.uiManager && this.uiManager.resetBtn) {
                                this.uiManager.resetBtn.classList.add(
                                    'draw-requested'
                                );
                                this.uiManager.rerollBtn &&
                                    this.uiManager.rerollBtn.classList.remove(
                                        'draw-requested'
                                    );
                            }
                            if (
                        ...
                        console.warn(
                            'Ignored error in reset click handler (game.js)',
                            e
                        );
                    }
                });
            ...
            if (this.uiManager.setRerollEnabled)
                this.uiManager.setRerollEnabled(true);
            if (this.uiManager.setResetEnabled)
                this.uiManager.setResetEnabled(true);
            if (this.uiManager.setDrawEnabled)
                this.uiManager.setDrawEnabled(true);
            if (this.uiManager.setForfeitEnabled)
                this.uiManager.setForfeitEnabled(true);
        }
    ...
    startOTBGame() {
        ...
        this.currentController = new HotseatController(
            this.pieces,
            this.renderer,
            this.uiManager,
            this.seed
    ...
    startOnlineSearch() {
        ...
        if (this.uiManager) {
            ...
            if (this.uiManager.setRerollEnabled)
                this.uiManager.setRerollEnabled(false);
            if (this.uiManager.setResetEnabled)
                this.uiManager.setResetEnabled(false);
        }
        ...
        try {
            ...
            this.onlineSocket.onmessage = (ev) => {
                ...
                if (data.type === 'WAITING') {
                    if (this.uiManager)
                        this.uiManager.showMessage(data.message || '⏳', 2000);
                } else if (data.type === 'MATCHED') {
                    // Deserialize pieces from server
                    const deserialized = PieceSerializer.deserialize(
                        data.pieces
                    );
                    ...
                    this.currentController = new OnlineGameController(
                        this.pieces,
                        this.renderer,
                        this.uiManager,
                        this.onlineSocket,
                    ...
                    if (
                        this.currentController &&
                        typeof this.currentController.applyRemoteMove ===
                            'function'
                    ) {
                    ...
                    if (this.takebackLocalRequest) {
                        try {
                            ...
                            console.warn(
                                'Ignored error responding to TAKEBACK_REQUEST (game.js)',
                                e
                            );
                        }
                        // inform opponent
                    ...
                    try {
                        ...
                        console.warn(
                            'Ignored error performing TAKEBACK_PERFORM locally (game.js)',
                            e
                        );
                    }
                    this.clearTakebackRequests();
                    ...
                    if (
                        this.uiManager &&
                        typeof this.uiManager.setDrawRequested === 'function'
                    )
                        this.uiManager.setDrawRequested(
                            this.drawLocalRequest,
                            true
                        );
                    ...
                    if (this.drawLocalRequest) {
                        try {
                            ...
                            try {
                                ...
                                console.warn(
                                    'Ignored error deactivating controller (game.js)',
                                    e
                                );
                            }
                        } catch (e) {
                            console.warn(
                                'Ignored error while handling DRAW_REQUEST agree flow (game.js)',
                                e
                            );
                        }
                        if (
                    ...
                    try {
                        ...
                        console.warn(
                            'Ignored error finishing DRAW_PERFORM (game.js)',
                            e
                        );
                    }
                    if (this.uiManager) this.uiManager.showMessage('🤝', 0);
                    ...
                    try {
                        ...
                        console.warn(
                            'Ignored error updating REMATCH_STATUS UI (game.js)',
                            e
                        );
                    }
                    this.clearDrawRequests();
                    ...
                    try {
                        if (this.uiManager) {
                            this.rematchLocalSelection =
                                data.mySelection || null;
                            this.rematchOpponentSelection =
                                data.opponentSelection || null;
                            // Update visual state on reroll/reset buttons using draw-requested classes
                            if (this.uiManager.rerollBtn)
                                this.uiManager.rerollBtn.classList.toggle(
                                    'draw-requested',
                                    this.rematchLocalSelection === 'roll'
                                );
                            if (this.uiManager.resetBtn)
                                this.uiManager.resetBtn.classList.toggle(
                                    'draw-requested',
                                    this.rematchLocalSelection === 'keep'
                                );
                            if (this.uiManager.rerollBtn)
                                this.uiManager.rerollBtn.classList.toggle(
                                    ...
                                    this.rematchOpponentSelection === 'roll'
                                );
                            if (this.uiManager.resetBtn)
                                this.uiManager.resetBtn.classList.toggle(
                                    'draw-opponent-requested',
                                    this.rematchOpponentSelection === 'keep'
                                );
                        }
                    } catch (e) {
                        console.warn(
                            'Ignored error updating REMATCH_STATUS visual state (game.js)',
                            e
                        );
                    }
                    ...
                    try {
                        ...
                        const placement =
                            typeof data.placement !== 'undefined'
                                ? data.placement
                                : null;
                        ...
                        if (
                            ...
                            try {
                                ...
                                this.pieces =
                                    PieceGenerator.generateRandomPieces(
                                        this.seed
                                    );
                                try {
                                    if (
                                        typeof PieceGenerator.generatePlacement ===
                                        'function'
                                    ) {
                                        // generate placement deterministically from the seed
                                        const genPlacement =
                                            PieceGenerator.generatePlacement(
                                                this.pieces,
                                                this.seed
                                            );
                                    ...
                                    console.warn(
                                        'Placement generation error during rematch (game.js)',
                                        e
                                    );
                                }
                            } catch (e) {
                                console.warn(
                                    'Piece regeneration error during rematch (game.js)',
                                    e
                                );
                            }
                        }
                        ...
                        try {
                            ...
                            const prevColor =
                                this.currentController &&
                                this.currentController.playerColor
                                    ? this.currentController.playerColor
                                    : null;
                            ...
                            const newPlacement =
                                ...
                                (this.currentController &&
                                    this.currentController.engine &&
                                    this.currentController.engine.placement) ||
                                null;
                            this.currentController = new OnlineGameController(
                                this.pieces,
                                this.renderer,
                            ...
                            this.renderer.attachEventListener((row, col) =>
                                this.currentController.handleSquareClick(
                                    row,
                                    col
                                )
                            );
                            ...
                            console.warn(
                                'Failed to restart after rematch result',
                                e
                            );
                        }
                    ...
                    if (this.uiManager) {
                        ...
                        try {
                            ...
                            console.warn(
                                'Ignored error finishing REMATCH_RESULT flow (game.js)',
                                e
                            );
                        }
                    }
            ...
            this.onlineSocket.onerror = (e) => {
                if (this.uiManager) this.uiManager.showMessage('⚠️', 2000);
            };
    ...
    clearTakebackRequests() {
        ...
        if (
            this.uiManager &&
            typeof this.uiManager.setTakebackRequested === 'function'
        )
            this.uiManager.setTakebackRequested(false, false);
    }
    ...
    clearDrawRequests() {
        ...
        if (
            ...
            this.uiManager.setDrawRequested(false, false);
    }
    ...
    setupMultiplayerCallbacks() {
        ...
        this.multiplayerClient.onMatchFound = (
            color,
            pieces,
            placement,
            moveHistory = [],
            ...
            if (Array.isArray(moveHistory) && moveHistory.length > 0) {
                for (const entry of moveHistory) {
                    try {
                        ...
                        console.warn('Failed to replay move', e);
                    }
                }
            }
            ...
            if (this.uiManager) {
                ...
                if (this.uiManager.hideEndmatchControls)
                    this.uiManager.hideEndmatchControls();
            }
        };
        ...
        this.multiplayerClient.onRematchStatus = (
            mySelection,
            opponentSelection
        ) => {
            if (this.uiManager) {
                this.uiManager.setRematchSelections(
                    mySelection,
                    opponentSelection
                );
            }
        ...
        this.multiplayerClient.onOpponentLeft = (data) => {
            ...
            setTimeout(() => {
                ...
                if (
                    !this.multiplayerClient ||
                    !this.multiplayerClient.ws ||
                    this.multiplayerClient.ws.readyState === WebSocket.CLOSED
                ) {
