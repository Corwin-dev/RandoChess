...
function matchPlayer(ws, pieces, seed = null) {
    if (waitingQueue.length > 0) {
        ...
        if (!selectedPieces) {
            try { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'ERROR', message: 'Invalid pieces' })); } catch (e) { console.warn('Failed to send ERROR to client', e); }
            return;
        }
        ...
        try { if (waiting.ws.readyState === WebSocket.OPEN) waiting.ws.send(JSON.stringify(msg0)); } catch (e) { console.warn('Failed to send MATCHED to waiting client', e); }
        try { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(msg1)); } catch (e) { console.warn('Failed to send MATCHED to joining client', e); }
    } else {
        waitingQueue.push({ ws, pieces, seed });
        try { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'WAITING', message: '⏳' })); } catch (e) { console.warn('Failed to send WAITING to client', e); }
    }
}

wss.on('connection', (ws) => {
    ws.playerId = Math.random().toString(36).substring(2, 12).toUpperCase();
    try { ws.send(JSON.stringify({ type: 'ASSIGN_ID', playerId: ws.playerId })); } catch (e) { console.warn('Failed to assign id to client', e); }
    ...
    ws.on('message', (msg) => {
        ...
        if (data.type === 'JOIN_QUEUE') {
            ...
            try { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'LEFT_QUEUE' })); } catch (e) { console.warn('Failed to notify client LEFT_QUEUE', e); }
        } else if (data.type === 'MOVE') {
            const session = gameSessions.get(ws.sessionId);
            if (!session) return;
            // broadcast to both players
            for (const p of session.players) {
                try { if (p.readyState === WebSocket.OPEN) p.send(JSON.stringify(Object.assign({ type: 'MOVE' }, { move: data.move, gameOver: data.gameOver || false, winner: data.winner || null }))); } catch (e) { console.warn('Failed to forward MOVE to session player', e); }
            }
            ...
            for (const p of session.players) {
                if (p === ws) continue;
                try { if (p.readyState === WebSocket.OPEN) p.send(JSON.stringify(data)); } catch (e) { console.warn('Failed to forward control message to opponent', e); }
            }
            ...
            for (const p of session.players) {
                try {
                    ...
                    const payload = { type: 'REMATCH_STATUS', mySelection: votes[p.playerId] || null, opponentSelection: other ? (votes[other.playerId] || null) : null };
                    p.send(JSON.stringify(payload));
                } catch (e) { console.warn('Error while handling REMATCH_VOTE resolution', e); }
            }
            ...
            if (playerIds.every(id => votes[id])) {
                ...
                if (action === 'reroll') {
                    ...
                    payloadExtra.seed = (typeof session.seed !== 'undefined') ? session.seed : null;
                }

                // Notify both players of rematch result
                for (const p of session.players) {
                    try {
                        if (p.readyState === WebSocket.OPEN) p.send(JSON.stringify(Object.assign({ type: 'REMATCH_RESULT', action, placement: newPlacement }, payloadExtra)));
                    } catch (e) { console.warn('Failed to notify player of rematch result', e); }
                }
    ...
    ws.on('close', () => {
        ...
        if (ws.sessionId && gameSessions.has(ws.sessionId)) {
            const session = gameSessions.get(ws.sessionId);
            session.players.forEach(p => { if (p !== ws && p.readyState === WebSocket.OPEN) { try { p.send(JSON.stringify({ type: 'OPPONENT_LEFT' })); } catch (e) { console.warn('Failed to notify opponent that player left', e); } } });
