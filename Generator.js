...
class PieceGenerator {
    ...
    static isDirectionallyRestricted(moves) {
        ...
        return (
            !hasBackward && !expandedHasBackward && (hasForward || onlyVertical)
        );
    }
    ...
    static getUpgradeBonusRange(moves) {
        ...
        if (!hasAnyForward || hasAnyBackward || expandedHasBackward)
            return null;

        const hasInfiniteForward = baseInfo.some(
            ({ dy, move }) =>
                dy > 0 && move.distance === -1 && move.jump !== 'required'
        );

        const hasDoubleForward = baseInfo.some(
            ({ dy, move }) => dy > 0 && move.distance === 2
        );
        ...
        const hasHorizontal = baseInfo.some(
            ({ dx, dy }) => dx !== 0 && dy === 0
        );
        if (hasHorizontal) {
    ...
    static generateUpgradeMoves(originalMoves, rng, bonusRange = null) {
        ...
        while (this.isDirectionallyRestricted(newMoves) && attemptsLeft > 0) {
            ...
            const symmetry =
                this.symmetries[
                    Math.floor(rng.next() * this.symmetries.length)
                ];
            const isStraightMove = dx === 0 || dy === 0 || dx === dy;
            const jump =
                isStraightMove || isOrthogonal ? 'prohibited' : 'required';
            const distance =
                jump === 'required' ? 1 : rng.next() < 0.5 ? -1 : 1;
        ...
        const bonusCount =
            Math.floor(rng.next() * (bonusMax - bonusMin + 1)) + bonusMin;
    ...
    static generateRandomPieces(seed = null) {
        ...
        const pieceSignature = (p) => {
            const mv = (m) =>
                `${m.step[0]},${m.step[1]}|${m.symmetry}|${m.distance}|${m.jump}|${m.requiresUnmoved}|${m.capture}`;
            const ups = (p.upgradeMoves || []).map(mv).sort().join(';');
            const moves = (p.moves || []).map(mv).sort().join(';');
            const specs = (p.specials || [])
                .map((s) => s.type)
                .sort()
                .join(',');
            return `${p.royal ? 'R' : 'N'}|${
                p.promotionType || ''
            }|${specs}|MOVES:${moves}|UP:${ups}`;
        };
        ...
        try {
            ...
            const absHas = (ax, ay) =>
                allSteps.some(
                    ([sx, sy]) => Math.abs(sx) === ax && Math.abs(sy) === ay
                );
            ...
            for (const [jx, jy] of jumpCandidates) {
                ...
                if (!interPresent && !jumpPresent) {
                    ...
                    royalMoves.push(
                        new Move([jx, jy], symR, 1, 'required', false)
                    );
                }
                ...
                const nowJumpPresent = tmpSteps.some(
                    ([sx, sy]) =>
                        Math.abs(sx) === Math.abs(jx) &&
                        Math.abs(sy) === Math.abs(jy)
                );
                if (nowJumpPresent) {
        ...
        } catch (e) { console.warn('Ignored error (Generator.js)', e); }

        const royalSymbol = this.selectSymbolForPiece(
            royalMoves,
            true,
            false,
            usedSymbols
        ...
        const createNonRoyal = (moves) => {
            const symbol = this.selectSymbolForPiece(
                moves,
                false,
                false,
                usedSymbols,
            ...
            if (this.isDirectionallyRestricted(moves)) {
                ...
                upgradeMoves = this.generateUpgradeMoves(
                    moves,
                    rng,
                    bonusRange
                );
        ...
        let slider;
        {
            ...
            do {
                ...
                if (!usedSignatures.has(sig)) {
                    slider = candidate;
                    let hasUnlimited = slider.moves.some(
                        (m) => m.distance === -1 && m.jump !== 'required'
                    );
                    if (!hasUnlimited) {
                        const slideIdx = slider.moves.findIndex(
                            (m) => m.jump !== 'required'
                        );
                        if (slideIdx >= 0) {
                    ...
                    const hasNonHorizontal = slider.moves.some(
                        (m) => m.symmetry && m.symmetry !== 'Horizontal'
                    );
                    if (!hasNonHorizontal) {
                        const idx = slider.moves.findIndex(
                            (m) => m.jump !== 'required'
                        );
                        if (idx >= 0) {
                    ...
                    if (this.isDirectionallyRestricted(slider.moves)) {
                        slider.promotionRank = this.getFarthestReachableRank(
                            slider.moves
                        );
                        slider.promotionType = 'move-upgrade';
                        const bonusRange = this.getUpgradeBonusRange(
                            slider.moves
                        );
                        slider.upgradeMoves = this.generateUpgradeMoves(
                            slider.moves,
                            rng,
                            bonusRange
                        );
            ...
            if (!slider) {
                ...
                if (
                    ...
                    if (slideIdx >= 0) slider.moves[slideIdx].distance = -1;
                        ...
                        slider.moves.push(
                            new Move([1, 0], '4way', -1, 'prohibited', false)
                        );
                }
                if (
                    !slider.moves.some(
        ...
        let trickster;
        {
            ...
            do {
                ...
                const hasJump = candidate.moves.some(
                    (m) => m.jump === 'required'
                );
                if (!hasJump) {
                    candidate.moves.push(
                        new Move([2, 1], '4way', 1, 'required', false)
                    );
                }
                ...
                if (hasE && hasW && hasN && hasS) {
                    for (const m of candidate.moves) {
                        const steps = m.getSteps();
                        const producesCardinal = steps.some(
                            ([dx, dy]) =>
                                (dx === 1 && dy === 0) ||
                                (dx === -1 && dy === 0) ||
                                (dx === 0 && dy === 1) ||
                ...
                if (this.isDirectionallyRestricted(candidate.moves)) {
                    candidate.promotionRank = this.getFarthestReachableRank(
                        candidate.moves
                    );
                    ...
                    candidate.upgradeMoves = this.generateUpgradeMoves(
                        candidate.moves,
                        rng,
                        bonusRange
                    );
            ...
            if (!trickster) {
                ...
                if (this.isDirectionallyRestricted(trickster.moves)) {
                    trickster.promotionRank = this.getFarthestReachableRank(
                        trickster.moves
                    );
                    ...
                    trickster.upgradeMoves = this.generateUpgradeMoves(
                        trickster.moves,
                        rng,
                        bonusRange
                    );
        ...
        let flower;
        {
            ...
            do {
                ...
                try {
                    ...
                    const existingSymmetry = candidate.moves.find(
                        (m) => m.symmetry === '8way'
                    )
                        ? '8way'
                        : candidate.moves.find((m) => m.symmetry === '4way')
                        ? '4way'
                        : null;
                    for (const [jx, jy] of jumpCandidates) {
                        const interAx = Math.abs(jx / 2);
                        ...
                        if (!interPresent && !jumpPresent) {
                            ...
                            if (!sym)
                                sym =
                                    Math.abs(jx) === Math.abs(jy)
                                        ? '8way'
                                        : '4way';
                            candidate.moves.push(
                                new Move([jx, jy], sym, 1, 'required', false)
                            );
                        }
                        ...
                        if (!jumpPresent) {
                            ...
                            if (
                                ...
                                candidate.moves = candidate.moves.filter(
                                    (m) => {
                                        const steps = m.getSteps();
                                        return !steps.some(
                                            ([sx, sy]) =>
            ...
            if (!flower) {
                ...
                try {
                    ...
                    for (const [jx, jy] of jumpCandidates) {
                        ...
                        if (!interPresent && !jumpPresent) {
                            const existingSymmetryF = flower.moves.find(
                                (m) => m.symmetry === '8way'
                            )
                                ? '8way'
                                : flower.moves.find(
                            ...
                            if (!symF)
                                symF =
                                    Math.abs(jx) === Math.abs(jy)
                                        ...
                                        : '4way';
                            flower.moves.push(
                                new Move([jx, jy], symF, 1, 'required', false)
                            );
                        }
                ...
                if (this.isDirectionallyRestricted(flower.moves)) {
                    flower.promotionRank = this.getFarthestReachableRank(
                        flower.moves
                    );
                    flower.promotionType = 'move-upgrade';
                    const bonusRange = this.getUpgradeBonusRange(flower.moves);
                    flower.upgradeMoves = this.generateUpgradeMoves(
                        flower.moves,
                        rng,
                        bonusRange
                    );
        ...
        if (!hasKnight) {
            const target =
                typeof trickster !== 'undefined' && trickster
                    ? trickster
                    : pieces[1];
            if (target) {
                target.moves.push(
        ...
        const promotionPieces = pieces.filter(
            (p) => !p.royal && p.promotionType !== 'choice'
        );

        const pawnSymbol = this.selectSymbolForPiece(
            pawnMoves,
            false,
            true,
            usedSymbols,
    ...
    static getUniqueSymbol(symbolArray, usedSymbols, random = Math.random) {
        ...
        if (availableSymbols.length > 0) {
            return availableSymbols[
                Math.floor(random() * availableSymbols.length)
            ];
        }

        const genericSymbols = ['★', '☆', '✫', '✬', '✭', '✮', '✯'];
        const availableGeneric = genericSymbols.filter(
            (s) => !usedSymbols.has(s)
        );

        if (availableGeneric.length > 0) {
            return availableGeneric[
                Math.floor(random() * availableGeneric.length)
            ];
        }
    ...
    static generateRandomMoves(count, isRoyal = false, rng = null) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const symmetry = isRoyal
                ? '4way'
                : this.symmetries[
                      Math.floor(random() * this.symmetries.length)
                  ];
            ...
            moves.push(
                new Move([dx, dy], symmetry, distance, jump, requiresUnmoved)
            );
        }
        ...
        if (!hasYMovement && moves.length > 0) {
            ...
            let dy = stepOptions.filter((v) => v !== 0)[
                Math.floor(random() * stepOptions.filter((v) => v !== 0).length)
            ];
            ...
            const symmetry =
                this.symmetries[Math.floor(random() * this.symmetries.length)];
            const isStraightMove = dx === 0 || dy === 0 || dx === dy;
            const jump =
                isStraightMove || isOrthogonal ? 'prohibited' : 'required';
            const distance =
                jump === 'required'
                    ...
                    : random() < 0.5
                    ? -1
                    : Math.floor(Math.pow(random(), 2) * 3) + 1;
    ...
    static createMovementPatternIcon(
        piece,
        size = 80,
        color = 'white',
        ...
        for (const move of piece.moves) {
            ...
            for (const [dx, dy] of steps) {
                ...
                for (let dist = 1; dist <= maxDist; dist++) {
                    const shouldFlip =
                        (playerPerspective === 'black' && color === 'black') ||
        ...
        if (piece.upgradeMoves && piece.upgradeMoves.length > 0) {
            for (const move of piece.upgradeMoves) {
                ...
                for (const [dx, dy] of stepsU) {
                    const maxDistU =
                        move.distance === -1 ? gridSize : move.distance;
    ...
    static buildMoveTable(engine) {
        ...
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                ...
                const symbol =
                    cell.piece && cell.piece.name ? cell.piece.name : 'unknown';

                let moves = [];
                try {
                    moves =
                        typeof engine.getValidMoves === 'function'
                            ? engine.getValidMoves(r, c)
                            : [];
                } catch (e) {
                    moves = [];
                }
    ...
    static computeMaterialAdvantage(engine) {
        ...
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                const cell = engine.board[r][c];
                if (!cell) continue;
                const val = this.getStaticPieceValue(
                    cell.piece,
                    r,
                    c,
                    cell.color,
    ...
    static getStaticPieceValue(piece, row, col, color, engine = null) {
        ...
        if (piece.promotionPieces && piece.promotionPieces.length > 0) {
            ...
            if (distanceToPromotion < 3)
                value += (3 - distanceToPromotion) * 0.5;
        }
